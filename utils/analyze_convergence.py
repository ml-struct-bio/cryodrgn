...
def add_args(parser):
    ...
    group.add_argument(
        '--random-seed', default=None, help='Manually specify the seed used for selection of subset particles'
    )
    group.add_argument(
        '--random-state',
        type=int,
    ...
    group.add_argument(
        '--smooth', type=bool, default=True, help='smooth the 2D histogram before identifying local maxima'
    )
    group.add_argument(
        '--smooth-width',
        type=float,
    ...
    group.add_argument(
        '--pruned-maxima', type=int, default=12, help='prune poorly-separated maxima until this many maxima remain'
    )
    group.add_argument(
        '--radius',
        type=float,
    ...
    group.add_argument(
        '--max-threads', type=int, default=8, help='Max number of threads used to parallelize mask generation'
    )
    group.add_argument(
        '--thresh',
        type=float,
...
def plot_loss(logfile, outdir, E, LOG):
    ...
    plt.savefig(outdir + '/plots/00_total_loss.png', dpi=300, format='png', transparent=True, bbox_inches='tight')
    flog(f'Saved total loss plot to {outdir}/plots/00_total_loss.png', LOG)


def encoder_latent_umaps(
    workdir, outdir, epochs, n_particles_total, subset, random_seed, use_umap_gpu, random_state, n_epochs_umap, LOG
):
    """
...
def sketch_via_umap_local_maxima(
    outdir, E, LOG, n_bins=30, smooth=True, smooth_width=1, pruned_maxima=12, radius=5, final_maxima=10
):
    """
    ...
    for i, ax in enumerate(axes.flat):
        if to_plot[i] == 'umap':
            ...
            for k in range(len(values)):
                ax.text(x=coords[k, 0] + 0.3, y=coords[k, 1] + 0.3, s=labels[k], fontdict=dict(color='r', size=10))
    ...
    plt.savefig(
        f'{outdir}/plots/03_decoder_UMAP-sketching.png', dpi=300, format='png', transparent=True, bbox_inches='tight'
    )
...
def follow_candidate_particles(workdir, outdir, epochs, n_dim, binned_ptcls_mask, labels, LOG):
    ...
    for i, ax in enumerate(axes.flat):
        try:
            ...
            z_maxima_median_ondata, z_maxima_median_ondata_ind = analysis.get_nearest_point(
                z, z_maxima_median
            ...
            ax.scatter(
                umap_maxima_median_ondata[:, 0], umap_maxima_median_ondata[:, 1], s=10, linewidth=0, c='r', alpha=1
            )
            ax.set_title(f'epoch {epochs[i]}')
        except IndexError:
            pass
...
def generate_volumes(workdir, outdir, epochs, Apix, flip, invert, downsample, cuda, LOG):
    ...
    for epoch in epochs:
        ...
        analysis.gen_volumes(
            weights, config, zfile, volsdir, Apix=Apix, flip=flip, invert=invert, downsample=downsample, cuda=cuda
        )
...
def calculate_FSCs(outdir, epochs, labels, img_size, chimerax_colors, LOG):
    ...
    plt.savefig(
        f'{outdir}/plots/07_decoder_FSC-nyquist.png', dpi=300, format='png', transparent=True, bbox_inches='tight'
    )
...
def main(args):
    ...
    flog(
        f'Convergence 3: calculating and plotting latent encoding vector shifts for all epochs up to epoch {E} ...', LOG
    )
