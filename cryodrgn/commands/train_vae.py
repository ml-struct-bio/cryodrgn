...
def add_args(parser):
    ...
    group.add_argument(
        '--num-workers-per-gpu', type=int, default=4, help='Number of num_workers of Dataloader (default: %(default)s)'
    )
    group.add_argument(
        '--max-threads',
        type=int,
    ...
    group.add_argument(
        '-n', '--num-epochs', type=int, default=20, help='Number of training epochs (default: %(default)s)'
    )
    ...
    group.add_argument(
        '--beta', default=None, help='Choice of beta schedule or a constant for KLD weight (default: 1/zdim)'
    )
    group.add_argument(
    ...
    group.add_argument(
        '--pose-lr', type=float, default=3e-4, help='Learning rate for pose optimizer (default: %(default)s)'
    )
    ...
    group.add_argument(
        '--enc-dim', dest='qdim', type=int, default=1024, help='Number of nodes in hidden layers (default: %(default)s)'
    )
    group.add_argument(
        '--encode-mode',
        default='resid',
    ...
    group.add_argument(
        '--dec-dim', dest='pdim', type=int, default=1024, help='Number of nodes in hidden layers (default: %(default)s)'
    )
    group.add_argument(
        '--pe-type',
...
def loss_function(z_mu, z_logvar, y, yt, y_recon, mask, beta, y_recon_tilt=None, beta_control=None):
    ...
    if use_tilt:
        gen_loss = 0.5 * gen_loss + 0.5 * F.mse_loss(y_recon_tilt, yt.view(B, -1)[:, mask])
    # latent loss
...
def eval_z(model, lattice, data, batch_size, device, trans=None, use_tilt=False, ctf_params=None, use_real=False):
    ...
    for minibatch in data_generator:
        ...
        if trans is not None:
            ...
            if yt is not None:
                yt = lattice.translate_ht(yt.view(B, -1), trans[ind].unsqueeze(1)).view(B, D, D)
        if use_real:
        ...
        for minibatch in data_generator:   # minibatch: [y, ind]
            ...
            if batch_it % args.log_interval == 0:
                log(
                    ...
                    'loss={:.6f}'.format(epoch + 1, num_epochs, batch_it, Nimg, gen_loss, kld, beta, loss)
                )
        flog(
