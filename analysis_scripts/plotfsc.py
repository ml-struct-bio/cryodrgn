...
def parse_args():
    ...
    parser.add_argument('-i', '--input', nargs='+', help='input cryoDRGN fsc text files', required=True)
    parser.add_argument('-a', '--angpix', type=float, default=0, help='physical pixel size in angstrom')
...
def main(args):
    ...
    ax.set_aspect(.3)  # Set the aspect ratio on the plot specifically
    
    if args.angpix != 0:
        freq = np.arange(1,6)*.1
        res = ['1/{:.1f}'.format(val) for val in ((1/freq)*args.angpix)]
        print(res)
        res_text = res
        plt.xticks(np.arange(1,6)*.1, res_text)
