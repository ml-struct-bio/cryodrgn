...
import fft
import lie_tools
from models import Lattice, BNNBHomo, BNBHomo, FTSliceDecoder, ResidLinearDecoder
...
def main(args):
    ...
    bnnb = BNNBHomo(model, lattice, tilt)
    bnb = BNBHomo(model, lattice, tilt)
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for minibatch_i in np.array_split(np.random.permutation(Nimg),num_batches):
            ...
            with torch.no_grad():
                if epoch < 1:
                    rot = bnnb.opt_theta(y,L,yt)
                else:
                    rot = bnb.opt_theta(y,args.l_start,args.l_end,yt)
            model.train()
