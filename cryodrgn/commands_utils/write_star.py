...
def main(args):
    ...
    assert input_ext in ('.mrcs', '.txt', '.star'), 'Input file must be .mrcs/.txt/.star'

    # Either accept an input star file, or an input .mrcs/.txt with optional ctf/pose pkl file(s)
    starfile = None
    if input_ext == '.star':
        assert args.poses is None, '--poses cannot be specified when input is a starfile (poses are obtained from starfile)'
        assert args.ctf is None, '--ctf cannot be specified when input is a starfile (ctf information are obtained from starfile)'

        starfile = Starfile.load(args.particles)
        particles = starfile.get_particles(datadir=os.path.dirname(args.particles), lazy=True)
