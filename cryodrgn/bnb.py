...
class BNBHomoBase:
    ...
    def keep_matrix(self, bound, Lstar, max_poses):
        ...
        if (Np > max_poses).any():
            ...
            maxQ = keep.shape[1]
            keep[torch.arange(B, dtype=torch.long).repeat_interleave(maxQ-max_poses), w] = 0
        if (Np == 0).any():
            log('WARNING: NO POSES BELOW BOUND') # this can be hit because of fl ulps
            log(Np)
            w = bound.argmin(1)
            keep[torch.arange(B),w] = 1
        return keep
...
class BNBHomo(BNBHomoBase):
    ...
    def __init__(self, decoder, lattice, Lmin, Lmax, tilt=None, t_extent=5, t_ngrid=7, nkeptposes=24, probabilistic=False):
        super().__init__()
        self.decoder = decoder
        self.lattice = lattice
        self.base_quat = so3_grid.base_SO3_grid()
    ...
    def opt_theta_trans(self, images,  images_tilt=None, niter=5):
        ...
        w = keep.nonzero().cpu() # sum(Np) x [img, trans_idx, rot_idx]
...
class BNBHet:
    ...
    def __init__(self, model, lattice, Lmin, Lmax, tilt=None, t_extent=5, t_ngrid=7, nkeptposes=24):
        self.model = model
        self.lattice = lattice
        self.base_quat = so3_grid.base_SO3_grid()
        self.base_rot = lie_tools.quaternions_to_SO3(torch.tensor(self.base_quat))
    ...
    def opt_theta(self, images, z, images_tilt=None, niter=5, probabilistic=False):
        ...
        keep = self.keep_matrix(bound, Lstar.view(B,1), self.nkeptposes, probabilistic) # BxQ
    ...
    def opt_theta_trans(self, images, z, images_tilt=None, niter=5, probabilistic=False):
        if probabilistic: # bound += B1 + B2
            raise NotImplementedError
        B = images.size(0)
        assert not self.model.training
        ...
        keep = self.keep_matrix(bound, Lstar.view(B,1), self.nkeptposes, probabilistic) # Bx-1
        keep = keep.view(B,len(self.base_shifts), self.nbase) # BxTxQ
        Np = keep.sum((-1,-2)) # per image # todo: filter keep by Np to max_poses
