...
def add_args(parser):
    ...
    group.add_argument(
        '--Apix', type=float, default=1, help='Pixel size to add to .mrc header (default: %(default)s A/pix)'
    )
    ...
    group.add_argument(
        '--thresh', type=float, help='Density value to threshold for masking (default: half of max density value)'
    )
    group.add_argument(
        '--dilate', type=int, default=5, help='Dilate initial mask by this amount (default: %(default)s pixels)'
    )
    group.add_argument(
        '--mask',
        metavar='MRC',
...
def analyze_volumes(
    outdir, K, dim, M, linkage, vol_ind=None, plot_dim=5, particle_ind_orig=None, Apix=1, vol_start_index=0
):
    cmap = choose_cmap(M)
    ...
    if not os.path.exists(f'{outdir}/kmeans{K}/vol_mean.mrc'):
        volm = np.array(
            [mrc.parse_mrc(f'{outdir}/kmeans{K}/vol_{vol_start_index+i:03d}.mrc')[0] for i in range(K)]
        ).mean(axis=0)
    ...
    for i in range(M):
        ...
        vol_i_std = np.average((vol_i_all - vol_i_mean) ** 2, axis=0, weights=nparticles) ** 0.5
    ...
    plt.scatter(umap[:, 0], umap[:, 1], alpha=0.1, s=1, rasterized=True, color='lightgrey')
