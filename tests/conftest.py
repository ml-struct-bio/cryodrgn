...
class TrainDir:
    ...
    def out_files(self) -> list[str]:
        """The output files that have been generated by this reconstruction model."""
    ...
    @property
    def volume_files(self) -> dict[int, str]:
        """Returns a dictionary mapping epoch numbers to their corresponding volume files.

        Returns
        -------
        dict[int, str]
            Dictionary where keys are epoch numbers and values are the paths to the
            corresponding reconstructed volume files.

        """
        volfiles = {epoch: None for epoch in self.epoch_iter}
...
class AbInitioDir:
    ...
    def backproject(self) -> None:
        ...
        in_poses = os.path.join(self.outdir, f"pose.{self.epochs}.pkl")
