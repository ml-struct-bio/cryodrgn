...
def add_args(parser):
    ...
    parser.add_argument(
        '--checkpoint', type=int, default=1, help='Checkpointing interval in N_EPOCHS (default: %(default)s)'
    )
    parser.add_argument(
        '--log-interval', type=int, default=1000, help='Logging interval in N_IMGS (default: %(default)s)'
    )
    ...
    group.add_argument(
        '--no-window', dest='window', action='store_false', help='Turn off real space windowing of dataset'
    )
    group.add_argument('--window-r', type=float, default=0.85, help='Windowing radius (default: %(default)s)')
    group.add_argument(
        '--datadir',
    ...
    group.add_argument(
        '--lazy-single', action='store_true', help='Lazy loading if full dataset is too large to fit in memory'
    )
    ...
    group.add_argument(
        '--tilt-deg', type=float, default=45, help='X-axis tilt offset in degrees (default: %(default)s)'
    )
    group.add_argument('--enc-only', action='store_true', help='Use the tilt pair only in VAE and not in BNB search')

    group = parser.add_argument_group('Training parameters')
    group.add_argument(
        '-n', '--num-epochs', type=int, default=30, help='Number of training epochs (default: %(default)s)'
    )
    ...
    group.add_argument(
        '--beta', default=1.0, help='Choice of beta schedule or a constant for KLD weight (default: %(default)s)'
    )
    group.add_argument(
        '--beta-control', type=float, help='KL-Controlled VAE gamma. Beta is KL target. (default: %(default)s)'
    )
    group.add_argument('--equivariance', type=float, help='Strength of equivariance loss (default: %(default)s)')
    group.add_argument(
        '--eq-start-it', type=int, default=100000, help='It at which equivariance turned on (default: %(default)s)'
    )
    group.add_argument(
        '--eq-end-it', type=int, default=200000, help='It at which equivariance max (default: %(default)s)'
    )
    group.add_argument(
        '--norm',
        type=float,
    ...
    group.add_argument(
        '--l-ramp-model', type=int, default=0, help='If 1, then during ramp only train the model up to l-max'
    )
    ...
    group.add_argument(
        '--t-extent', type=float, default=10, help='+/- pixels to search over translations (default: %(default)s)'
    )
    ...
    group.add_argument(
        '--ps-freq', type=int, default=5, help='Frequency of pose inference (default: every %(default)s epochs)'
    )
    group.add_argument(
        '--nkeptposes',
        type=int,
    ...
    group.add_argument(
        '--enc-layers', dest='qlayers', type=int, default=3, help='Number of hidden layers (default: %(default)s)'
    )
    group.add_argument(
        '--enc-dim', dest='qdim', type=int, default=256, help='Number of nodes in hidden layers (default: %(default)s)'
    )
    group.add_argument(
        '--encode-mode',
        default='resid',
    ...
    group.add_argument(
        '--use-real', action='store_true', help='Use real space image for encoder (for convolutional encoder)'
    )

    group = parser.add_argument_group('Decoder Network')
    group.add_argument(
        '--dec-layers', dest='players', type=int, default=3, help='Number of hidden layers (default: %(default)s)'
    )
    group.add_argument(
        '--dec-dim', dest='pdim', type=int, default=256, help='Number of nodes in hidden layers (default: %(default)s)'
    )
    group.add_argument(
        '--pe-type',
    ...
    group.add_argument(
        '--feat-sigma', type=float, default=0.5, help='Scale for random Gaussian features (default: %(default)s)'
    )
    ...
    group.add_argument(
        '--activation', choices=('relu', 'leaky_relu'), default='relu', help='Activation (default: %(default)s)'
    )
    return parser
...
def pretrain(model, lattice, optim, minibatch, tilt, zdim):
    ...
    if use_tilt:
        yt = yt.view(B, -1)[:, mask]
        gen_loss = 0.5 * F.mse_loss(gen_slice(rot), y) + 0.5 * F.mse_loss(gen_slice(tilt @ rot), yt)
    else:
...
def train(
    ...
    if use_tilt:
        gen_loss = 0.5 * F.mse_loss(gen_slice(rot), y) + 0.5 * F.mse_loss(gen_slice(bnb.tilt @ rot), yt)  # noqa: F821
    else:
        gen_loss = F.mse_loss(gen_slice(rot), y)
...
def save_checkpoint(
    model, lattice, optim, epoch, norm, search_pose, z_mu, z_logvar, out_mrc_dir, out_weights, out_z, out_poses
):
...
def save_config(args, dataset, lattice, model, out_config):
    ...
    config = dict(dataset_args=dataset_args, lattice_args=lattice_args, model_args=model_args)
...
def main(args):
    ...
    if args.tilt is None:
        ...
        if args.lazy:
            ...
            flog('Using preprocessed inputs. Ignoring any --window/--invert-data options')
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        flog(
            '# =====> Epoch: {} Average gen loss = {:.4}, KLD = {:.4f}, {}total loss = {:.4f}; Finished in {}'.format(
                epoch + 1, gen_loss_accum / Nimg, kld_accum / Nimg, eq_log, loss_accum / Nimg, dt.now() - t2
            )
        )
        ...
        if args.checkpoint and epoch % args.checkpoint == 0:
            ...
            with torch.no_grad():
                z_mu, z_logvar = eval_z(
                    model, lattice, data, args.batch_size, device, use_tilt=tilt is not None, ctf_params=ctf_params
                )
                save_checkpoint(
                    model,
                    lattice,
