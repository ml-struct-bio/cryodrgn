...
def test_pipeline(toy_projections_star, toy_projections_txt, toy_particles_mrcs, relion_mrcs, relion_starfile):
    ...
    args = parse_ctf_star.add_args(argparse.ArgumentParser()).parse_args(
        [toy_projections_star, '-D', '30', '--Apix', '1', '-o', 'output/pipeline_ctf.pkl']
    )
    ...
    args = write_star.add_args(argparse.ArgumentParser()).parse_args(
        [toy_particles_mrcs, '--ctf', 'output/pipeline_ctf.pkl', '-o', 'output/pipeline_test.star']
    )
    ...
    args = downsample.add_args(argparse.ArgumentParser()).parse_args(
        [toy_projections_star, '-D', '28', '--chunk', '80', '-o', 'output/toy_projections.mrcs']
    )
    ...
    args = write_star.add_args(argparse.ArgumentParser()).parse_args(
        [relion_mrcs, '--ctf', f'{DATA_FOLDER}/ctf1.pkl', '-o', 'output/pipeline_test3.star']
    )
    write_star.main(args)
