...
def add_args(parser):
    ...
    group.add_argument(
        '--window-r', default=0.85, type=float, help='Circular windowing mask inner radius (default: %(default)s)'
    )
    group.add_argument(
    ...
    group.add_argument(
        '--chunk', type=int, default=100000, help='Chunksize (in # of images) to split particle stack when saving'
    )
    group.add_argument(
        '--no-lazy', dest='lazy', action='store_false', help='Load whole dataset (faster than loading in batches)'
    )
    group.add_argument(
        '--max-threads',
        type=int,
...
def main(args):
    ...
    if downsample:
        assert args.D <= original_D, f'New box size {args.D} cannot be larger than the original box size {original_D}'
    ...
    def preprocess_numpy(imgs):
        ...
        with Pool(min(args.max_threads, mp.cpu_count())) as p:
            ...
            if window:
                imgs *= dataset.window_mask(original_D, args.window_r, 0.99, use_cupy=False)
