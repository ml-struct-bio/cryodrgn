...
def add_args(parser):
    ...
    parser.add_argument(
        '--ctf', metavar='pkl', type=os.path.abspath, help='CTF parameters (.pkl) for phase flipping images'
    )
    ...
    group.add_argument(
        '--tilt-deg', type=float, default=45, help='Right-handed x-axis tilt offset in degrees (default: %(default)s)'
    )
    return parser
    ...
    if args.tilt is None:
        if args.preprocessed:
            ...
            data = dataset.LazyMRCData(
                args.particles, norm=(0, 1), invert_data=args.invert_data, datadir=args.datadir, ind=args.ind
            )
        tilt = None
    else:   # tilt series
        if args.preprocessed:
        ...
        data = dataset.TiltMRCData(
            args.particles, args.tilt, norm=(0, 1), invert_data=args.invert_data, datadir=args.datadir, ind=args.ind
        )
