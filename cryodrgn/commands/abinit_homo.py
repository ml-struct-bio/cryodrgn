...
def add_args(parser):
    ...
    parser.add_argument('--window-r', type=float, default=0.85, help='Windowing radius (default: %(default)s)')
    ...
    group.add_argument(
        '--l-ramp-epochs', type=int, default=25, help='Number of epochs to ramp up to --l-end (default: %(default)s)'
    )
...
def pretrain(model, lattice, optim, batch, tilt=None):
    ...
    if tilt is not None:
        yt = yt.view(B, -1)[:, mask]
        loss = 0.5 * F.mse_loss(gen_slice(rot), y) + 0.5 * F.mse_loss(gen_slice(tilt @ rot), yt)
    else:
...
def train(model, lattice, ps, optim, batch, tilt_rot=None, no_trans=False, poses=None, base_pose=None, ctf_params=None):
    ...
    if tilt_rot is not None:
        loss = 0.5 * F.mse_loss(gen_slice(rot), y) + 0.5 * F.mse_loss(gen_slice(tilt_rot @ rot), yt)
    else:
        loss = F.mse_loss(gen_slice(rot), y)
    loss.backward()
    optim.step()
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        for batch in data_iterator:
            ...
            loss_item, pose, base_pose = train(
                model, lattice, ps, optim, batch, tilt, args.no_trans, poses=p, base_pose=bp, ctf_params=c
            )
