...
PARTICLES_3D_REQUIRED_COLUMNS = {
    ...
    "rlnVoltage", # rlnOpticsGroup
    "rlnSphericalAberration", # rlnOpticsGroup
    "rlnAmplitudeContrast", # rlnOpticsGroup
    "rlnImageSize", # rlnOpticsGroup
    "rlnImagePixelSize", # rlnOpticsGroup
}
...
# minimum output columns for 2D particles .star file (ordered)
PARTICLES_2D_OUTPUT_COLUMNS = [
    "_image_index",
    "_particle_index",
    "_tilt_index",
    "rlnImageName",
    "rlnGroupName", # rlnTomoParticleName
    "rlnImageSize",
    "rlnImagePixelSize",
    "rlnMicrographPreExposure",
    "rlnCtfScalefactor",
...
class Tomogram:
    ...
    def _translation_matrix(self, shift_3D_ang: np.ndarray) -> np.ndarray:
        """
        Build 4x4 translation matrix for "shift_3D_ang" in Angstroms.
        """
        mat = np.eye(4)
        mat[:3, 3] = shift_3D_ang
        return mat
...
=======
            r_0 = self._rotation_matrix([1, 0, 0], self.x_tilts[i])
            r_1 = self._rotation_matrix([0, 1, 0], self.y_tilts[i])
            r_2 = self._rotation_matrix([0, 0, 1], self.z_rots[i])
>>>>>>> d07e3c4 (refactor, comments, i/o validation)

            shift_3D_ang = np.array([
                self.x_shifts[i],
                self.y_shifts[i],
                0.0
            ])
            s_1 = self._translation_matrix(shift_3D_ang)
    ...
    def project_point(self, point_3d: np.ndarray, i_tilt: int) -> np.ndarray:
        """
    ...
    def calculate_local_defocus(self, i_tilt: int, point_3d: np.ndarray):
        R_x = R.from_euler("x", self.x_tilts[i_tilt], degrees=True)
        R_y = R.from_euler("y", self.y_tilts[i_tilt], degrees=True)
        R_z = R.from_euler("z", self.z_rots[i_tilt], degrees=True)
        ...
        depth_offset = (proj_pos[2] - proj_center[2]) * self.hand
    ...
    def expand_3D_particle_to_2D_particle(
        self,
        point_3d: np.ndarray,
        image_name: str,
        base_orientation_zyz: R | None = None,
    ):
...
=======
    ...
    for particles_3D_i_index, particles_3D_i in particles_3D_df.iterrows():
        ...
        _tomogram = Tomogram(
            ...
            defocus_u_array=tilt_series_i_df_visible["rlnDefocusU"].to_numpy(),
            defocus_v_array=tilt_series_i_df_visible["rlnDefocusV"].to_numpy(),
            defocus_angle_array=tilt_series_i_df_visible["rlnDefocusAngle"].to_numpy(),
            x_tilts=tilt_series_i_df_visible["rlnTomoXTilt"].to_numpy(),
            y_tilts=tilt_series_i_df_visible["rlnTomoYTilt"].to_numpy(),
            z_rots=tilt_series_i_df_visible["rlnTomoZRot"].to_numpy(),
            x_shifts=tilt_series_i_df_visible["rlnTomoXShiftAngst"].to_numpy(),
            y_shifts=tilt_series_i_df_visible["rlnTomoYShiftAngst"].to_numpy(),
            hand=tomograms_i_tomo_hand,
        )
...
>>>>>>> d07e3c4 (refactor, comments, i/o validation)
    ...
    print(f"generated metadata for {len(particles_2D_df)} images from {len(particles_3D_df)} 3D particles and {len(tomograms_df)} tomograms")
