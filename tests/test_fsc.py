...
def test_within_train(train_dir) -> None:
    """Test that we can compare two volumes produced during reconstruction training."""

    vol_file1 = os.path.join(train_dir.outdir, "reconstruct.0.mrc")
    vol_file2 = os.path.join(train_dir.outdir, "reconstruct.4.mrc")
    fsc_file = os.path.join(train_dir.outdir, "fsc.txt")
